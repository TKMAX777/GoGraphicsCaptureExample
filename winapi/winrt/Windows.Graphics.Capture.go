...
func (v *IDirect3D11CaptureFramePool) AddFrameArrived(eventHandler Direct3D11CaptureFramePoolVtbl) (*EventRegistrationToken, error) {
...
func (v *IDirect3D11CaptureFramePoolStatics) Create(device *IDirect3DDevice, pixelFormat DirectXPixelFormat, numberOfBuffers int32, size *SizeInt32) (*IDirect3D11CaptureFramePool, error) {
	...
	r1, _, _ := syscall.SyscallN(
		...
		uintptr(unsafe.Pointer(device)), uintptr(pixelFormat), uintptr(numberOfBuffers), uintptr(unsafe.Pointer(&size.Width)), uintptr(unsafe.Pointer(&ret)),
	)
	if r1 != win.S_OK {
		return nil, ole.NewError(r1)
	}
